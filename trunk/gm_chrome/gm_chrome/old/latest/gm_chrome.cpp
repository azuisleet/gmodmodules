...
LUA_FUNCTION(browser_draw)
{
	...
	if(!oneTime)
	{
		...
		HRESULT hr = g_pDevice->CreateVertexBuffer(sizeof(pVertices) * sizeof(PANELVERTEX),
			0,
			D3DFVF_PANELVERTEX,
			D3DPOOL_DEFAULT,
			&vertBuff,
		...
		VOID* pVerts;
		hr = vertBuff->Lock(0, sizeof(pVertices), (void**)&pVerts, 0);
		memcpy( pVerts, pVertices, sizeof(pVertices) );
		vertBuff->Unlock(); 
	}

	D3DXVECTOR3 vScreenCoord;
	GetScreenCoordFromWorld(g_pDevice, vScreenCoord, D3DXVECTOR3(0, 0, 0));
	gLua->Msg("0,0,0 screen pos %f %f %f\n", vScreenCoord.x, vScreenCoord.y, vScreenCoord.z);

	IDirect3DPixelShader9 *pshader = NULL;
	IDirect3DVertexShader9 *vshader = NULL;
	IDirect3DBaseTexture9 *texture = NULL;
	IDirect3DVertexBuffer9 *vertexbuffer = NULL;
	DWORD FVF, colorop, colorarg1, colorarg2;
	UINT offset, stride;

	g_pDevice->GetFVF( &FVF );
	g_pDevice->GetStreamSource(0, &vertexbuffer, &offset, &stride);
	...
	/* d3ddebug is telling me this is redundant*/
	//g_pDevice->SetTextureStageState( 0, D3DTSS_COLOROP,   D3DTOP_SELECTARG1 );
	//g_pDevice->SetTextureStageState( 0, D3DTSS_COLORARG1, D3DTA_TEXTURE );
	g_pDevice->SetStreamSource(0, vertBuff, 0, sizeof(PANELVERTEX));
	...
	g_pDevice->SetStreamSource(0, vertexbuffer, offset, stride);
