...
bool lookup_userid(int userid)
{
	...
	for (int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		if(engine->GetPlayerUserId(edict) == userid)
			return true;
	}
...
{
	INetChannel *channel = (INetChannel *)engine->GetPlayerNetInfo(engine->IndexOfEdict(pEntity));
	
	if(channel == NULL)
		return;

	NetFilter_ClientDisconnect(channel);
}
...
PLUGIN_RESULT CSSServerPlugin::ClientConnect( bool *bAllowConnect, edict_t *pEntity, const char *pszName, const char *pszAddress, char *reject, int maxrejectlen )
{
	...
	NetFilter_ClientConnect(channel, pEntity);
